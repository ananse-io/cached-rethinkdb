...
function validateUUID(idOrEntry, entryIdFn) {
  if (entryIdFn(idOrEntry)) {
    return Promise.resolve(entryIdFn(idOrEntry));
  }
...
class CachedRethinkDB {
  ...
  // DB Operation
  dbExist({reqId, id}) {
    return validateUUID(id, this.entryIdentifier).then(() => r.table(this.table).get(this.entryIdentifier(id)).run(this.dbConn)
    ).then((entryUuid) => {
      this.logger.trace({
        reqId,
        id,
  ...
  dbCreate({reqId, entry}) {
    return validateUUID(entry, this.entryIdentifier).then(() => r.table(this.table).insert(entry, {
      conflict: 'replace',
  ...
  dbRetrieve({reqId, id}) {
    return validateUUID(id, this.entryIdentifier).then(() => r.table(this.table).get(this.entryIdentifier(id)).run(this.dbConn)).then((entry) => {
      if (!entry) {
        this.logger.trace({
          reqId,
          id,
  ...
  dbUpdate({reqId, id, updateObjOrFn, nonAtomic = false}) {
    return validateUUID(id, this.entryIdentifier).then(() => {
      ...
      return r.table(this.table).get(this.entryIdentifier(id)).update(updateObjOrFn, { returnChanges: true, nonAtomic: !!nonAtomic }).run(this.dbConn);
    }).then((dbResult) => {
      if (dbResult.errors) {
  ...
  dbDelete({reqId, id}) {
    return validateUUID(id, this.entryIdentifier).then(() => r.table(this.table).get(this.entryIdentifier(id)).delete().run(this.dbConn)).then((dbResult) => {
      if (dbResult.errors) {
  ...
  create({reqId, entry}) {
    const action = 'create';
    return this.generateUUID({ reqId }).then((entryUUID) => {
